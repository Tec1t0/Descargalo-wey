// --- FUNCIONES Y VARIABLES GLOBALES DE LA CALCULADORA PRINCIPAL ---
        ...
        function calculateBisection(f_expr, a, b, max_iterations = 15) {
            ...
            try {
                ...
                // MENSAJE DE ERROR: Sintaxis de la función
                throw new Error(`Error de sintaxis en la función: ${e.message}`);
            }
            
            // Validación fundamental del Método de Bisección (Teorema de Bolzano)
            if (fa * fb >= 0) {
                // MENSAJE DE ERROR: Teorema de Bolzano no cumplido
                throw new Error(`f(a) y f(b) deben tener signos opuestos (Teorema de Bolzano). f(a)=${fa.toFixed(4)}, f(b)=${fb.toFixed(4)}. Producto: ${(fa * fb).toFixed(4)}.`);
            }
            ...
            // Asegurar que 'a' siempre sea el límite inferior para los cálculos internos
            if (a > b) {
            ...
            for (let n = 1; n <= max_iterations; n++) {
                ...
                try {
                    ...
                    // MENSAJE DE ERROR: Error de evaluación en iteración
                    throw new Error("Error durante la evaluación de la función. Revisa el punto y la sintaxis.");
                }

                // Cálculo del Error Relativo
                let relativeError = (m_previous !== null) ? Math.abs((m - m_previous) / m) * 100 : null;

                results.push({ n, a: current_a, b: current_b, m, fm, error: relativeError });

                // Actualizar el intervalo
                if (fa * fm < 0) {
                    current_b = m;
                    fb = fm; // Importante: actualiza el valor de f(b) para la siguiente comprobación
                } else if (fm * fb < 0) {
                    current_a = m;
                    fa = fm; // Importante: actualiza el valor de f(a) para la siguiente comprobación
                } else if (fm === 0) {
                    break; 
                }
        ...
        function displayResults(results) {
            ...
            summaryResult.innerHTML = `
                ...
                <p>Error Relativo Final: ${lastResult.error !== null ? lastResult.error.toFixed(4) + ' %' : 'N/A'}</p>
        ...
        window.onload = () => {
            ...
            document.getElementById('iterations').value = 6; // Usamos 10 iteraciones
